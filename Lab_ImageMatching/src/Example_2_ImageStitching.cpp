...
int main()
{
    // Load two images (here we assume that two images have the same size and type)
    const std::string input1 = std::string(DATA_DIR) + "hill01.jpg";
    const std::string input2 = std::string(DATA_DIR) + "hill02.jpg";
    cv::Mat image1 = cv::imread(input1);
    cv::Mat image2 = cv::imread(input2);
    ...
    cv::Mat merged;
    // Applies the perspective transformation to the 2nd image.
    ...
    cv::imshow("Image Stitching", merged);
    cv::waitKey(0);
