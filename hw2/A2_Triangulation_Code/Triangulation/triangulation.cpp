...
Triangulation::Triangulation(const std::string &title,
    ...
    : Viewer(title, 4, 3, 2, false, false), texture_0_(nullptr),
      texture_1_(nullptr), image_plane_(nullptr), view_frustum_(nullptr),
      show_images_(true) {
  auto load_image_points =
      [](const std::string image_point_file) -> std::vector<Vector2D> {
    PointCloud cloud;
  ...
  if (image_0_points_.empty())
    LOG(ERROR) << "failed loading image points from file: "
               << image_point_file_0;
  else
  ...
  if (image_0_points_.size() != image_1_points_.size())
    LOG(ERROR) << "numbers of image points do not match ("
               << image_0_points_.size() << " != " << image_1_points_.size()
               << ")";

  if (image_0_points_.size() < 8)
    LOG(ERROR) << "no enough point pairs (" << image_0_points_.size()
               << " loaded, but at least " << 8 << " needed)";
}

std::string Triangulation::usage() const {
  return ("\n============================== Usage "
          "====================================\n"
          "\tCorresponding images points should have been loaded (unless you "
          "see    \n"
          "\t\tsome error messages). If no errors, you can press the 'space' "
          "key    \n"
          "\t\tto run your 'triangulation()' method.                           "
          "     \n"
          "\tYou need to implement 'triangulation()' in "
          "'triangulation_method.cpp'. \n"
          "--------------------------------------------------------------------"
          "-----\n"
          "\tIf reconstruction was successful, the recovered 3D points will be "
          "     \n"
          "\tvisualized for you. You can use your mouse to interact with the "
          "viewer:\n"
          "\t\t- Left button: rotate                                           "
          "     \n"
          "\t\t- Right button: move                                            "
          ...
          "     \n"
          "\t\t- 'Ctrl' + 's': save the reconstructed 3D points into an '.xyz' "
          "file \n"
...
bool Triangulation::key_press_event(int key, int modifiers) {
  if (key == GLFW_KEY_SPACE) {
    ...
    const double fx = 1000; /// TODO: modify fx to see how it affects the
                            /// reconstruction (e.g., 970, 1010, 1030, 1050)
    const double fy =
        1000; /// TODO: modify fy to see how it affects the reconstruction
    const double cx = 320; /// TODO: modify cx to see how it affects the
                           /// reconstruction (e.g., 200, 250, 350, 400)
    const double cy =
        240; /// TODO: modify cy to see how it affects the reconstruction
    const double s = 0.0; /// TODO: modify s to see how it affects the
                          /// reconstruction (e.g., 1, 10, 100, 300, 500)
    LOG(ERROR) << "TODO: tune the parameters (above line " << __LINE__
               << " in file 'triangulation.cpp') and see how"
                  "\n\terrors in camera intrinsic parameters affect the final "
                  "reconstruction. Please include your findings"
                  "\n\tand insights in the report";
    const std::vector<double> fx_list({970, 1000, 1010, 1030, 1050});
    const std::vector<double> fy_list({970, 1000, 1010, 1030, 1050});
    const std::vector<double> cx_list({200, 250, 320, 350, 400});
    const std::vector<double> cy_list({180, 210, 240, 270, 300});
    const std::vector<double> s_list({0.0, 1, 10, 100, 300, 500});
          ...
          for (auto cy : cy_list) {
            for (auto s : s_list) {
              std::pair<bool, double> result =
                  triangulation(fx, fy, cx, cy, s, image_0_points_,
                                image_1_points_, points_3d, R, t);
              std::cout << "mean error: " << result.second << std::endl;
              if (result.second < error) {
                error = result.second;
                best_param = std::vector<double>({fx, fy, cx, cy, s});
                success = true;
              }
            }
          }
    ...
    std::cout << "fx: " << best_param[0] << std::endl;
    std::cout << "fy: " << best_param[1] << std::endl;
    std::cout << "cx: " << best_param[2] << std::endl;
    std::cout << "cy: " << best_param[3] << std::endl;
    std::cout << "s: " << best_param[4] << std::endl;
    std::pair<bool, double> result = triangulation(
        best_param[0], best_param[1], best_param[2], best_param[3],
        best_param[4], image_0_points_, image_1_points_, points_3d, R, t);
    if (result.first) {
      if (points_3d.empty()) {
        LOG(ERROR)
            << "triangulation() has returned 'true', but no 3D points returned";
        return false;
      }
      ...
      if (box.diagonal() > epsilon<double>()) {
        ...
        mat3 RR(R(0, 0), R(0, 1), R(0, 2), R(1, 0), R(1, 1), R(1, 2), R(2, 0),
                R(2, 1), R(2, 2));
        vec3 tt(t.data());
        ...
        LOG(ERROR) << "the reconstructed points has a diagonal length (of its "
                      "bounding box): "
                   << box.diagonal()
                   << ". This value is too small and the reconstruction might "
                      "not be correct";
        return false;
      }
    } else {
...
void Triangulation::post_draw() {
  ...
  const Rect quad0(x_min0, x_max0, 20 * dpi_scaling(),
                   (20 + tex0_h) * dpi_scaling());
  ...
  if (!current_model()) {
    const Rect quad1(x_min1, x_max1, 20 * dpi_scaling(),
                     (20 + tex1_h) * dpi_scaling());
...
void Triangulation::update_model(const std::vector<Vector3D> &points) {
  ...
  std::cout << "reconstructed model has " << cloud->n_vertices() << " points"
            << std::endl;
...
void Triangulation::update_image_plane(const mat3 &R, const easy3d::vec3 &t) {
  if (!image_plane_) {
    ...
    image_plane_->set_visible(show_images_);
    image_plane_->update_texcoord_buffer(
  ...
  const double fov = camera()->fieldOfView();
  const double hw_ratio = static_cast<double>(height()) / width();
  const double halfWidth = camera()->sceneRadius() *
                           2; // just a rought estimation, not accurate at all
  ...
  std::vector<vec3> corners = {
      vec3(-halfWidth, -halfHeight, -dist), vec3(halfWidth, -halfHeight, -dist),
      vec3(halfWidth, halfHeight, -dist), vec3(-halfWidth, halfHeight, -dist)};
